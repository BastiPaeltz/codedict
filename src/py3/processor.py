...
def start_process(cmd_line_args):
	...
	"""
	relevant_args = ({key: value for key, value in cmd_line_args.items() if value})
...
def read_config_write_on_failure(requested_item, section):
	...
	if not cfg_entry:
		...
		cfg_entry = input("Enter " +section+" :").strip()
		if section not in config.sections():
			config.add_section(section)	
...
def decide_where_to_print(all_results):
	...
	if len(all_results) < 10:
		...
		while True:
			choice = input("More than 10 results - print to console anyway? (y/n)").strip().split(" ")[0]
...
def update_content(body):
	...
	if body['<attribute>'] != 'DEL': 
		body['data'] = str(input("Change "+body['<attribute>']+" : ").strip(), 'utf-8')
...
def insert_content():
	...
	language = str(input("Enter language: ").strip(), 'utf-8')

	for index, item in enumerate(('shortcut: ', 'command: ', 'comment: ', 'link: ')): 
		content_to_add[index] = str(input("Enter "+ item).strip(), 'utf-8') 
